...
$${y}= {\beta~1~} +
{\beta~1~} \cdot {x~1~} +{\beta~2~} \cdot {x~2~}+{\beta~3~} \cdot {x~3~}....+{\beta~p~} \cdot {x~p~}+{ε}$$
where y is the response variable, x1 to Xp is the number of columns selected from 1 to p, β0 is the intercept of data, β1 to βp is the coefficients of corresponding columns of X from 1 to p. The ε is the error terms of the estimations.
